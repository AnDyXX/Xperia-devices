...
static inline int ax8swap_shmem_acct_size(unsigned long flags, loff_t size)
{
	return (flags & VM_NORESERVE) ?
		0 : ax8swap_security_vm_enough_memory_kern(VM_ACCT(size));
}
...
{
	return (flags & VM_NORESERVE) ?
		ax8swap_security_vm_enough_memory_kern(VM_ACCT(PAGE_CACHE_SIZE)) : 0;
}
...
int ax8swap_shmem_lock(struct file *file, int lock, struct user_struct *user)
{
	...
	if (lock && !(info->flags & VM_LOCKED)) {
		if (!ax8swap_user_shm_lock(inode->i_size, user))
			goto out_nomem;
		info->flags |= VM_LOCKED;
		mapping_set_unevictable(file->f_mapping);
	}
	if (!lock && (info->flags & VM_LOCKED) && user) {
		ax8swap_user_shm_unlock(inode->i_size, user);
		info->flags &= ~VM_LOCKED;
