...
void ax8swap_pud_clear_bad(pud_t *pud)
{
	pud_ERROR(*pud);
	pud_clear(pud);
}
 ...
 */
static void ax8swap_free_pte_range(struct mmu_gather *tlb, pmd_t *pmd)
{
...
static inline void ax8swap_free_pmd_range(struct mmu_gather *tlb, pud_t *pud,
				...
				unsigned long floor, unsigned long ceiling)
{
	...
	do {
		...
		if (pmd_none_or_clear_bad(pmd))
			continue;
		ax8swap_free_pte_range(tlb, pmd);
...
static inline void ax8swap_free_pud_range(struct mmu_gather *tlb, pgd_t *pgd,
				unsigned long addr, unsigned long end,
				unsigned long floor, unsigned long ceiling)
{
	pud_t *pud;
	...
	do {
		...
		if (pud_none_or_clear_bad(pud))
			continue;
		ax8swap_free_pmd_range(tlb, pud, addr, next, floor, ceiling);
 ...
 */
void ax8swap_free_pgd_range(struct mmu_gather *tlb,
			unsigned long addr, unsigned long end,
			unsigned long floor, unsigned long ceiling)
{
	pgd_t *pgd;
	...
	do {
		...
		if (pgd_none_or_clear_bad(pgd))
			continue;
		ax8swap_free_pud_range(tlb, pgd, addr, next, floor, ceiling);
	} while (pgd++, addr = next, addr != end);
}

void ax8swap_free_pgtables(struct mmu_gather *tlb, struct vm_area_struct *vma,
		unsigned long floor, unsigned long ceiling)
{
	while (vma) {
...
int ax8swap___pte_alloc(struct mm_struct *mm, pmd_t *pmd, unsigned long address)
{
...
int ax8swap___pte_alloc_kernel(pmd_t *pmd, unsigned long address)
{
 ...
 */
static void ax8swap_print_bad_pte(struct vm_area_struct *vma, unsigned long addr,
...
static inline int ax8swap_is_cow_mapping(unsigned int flags)
{
				...
				pte_t pte)
{
	...
	if (HAVE_PTE_SPECIAL) {
		...
		if (!(vma->vm_flags & (VM_PFNMAP | VM_MIXEDMAP)))
			ax8swap_print_bad_pte(vma, addr, pte, NULL);
		return NULL;
	}
	...
	if (unlikely(vma->vm_flags & (VM_PFNMAP|VM_MIXEDMAP))) {
		if (vma->vm_flags & VM_MIXEDMAP) {
			...
			if (pfn == vma->vm_pgoff + off)
				return NULL;
			if (!ax8swap_is_cow_mapping(vma->vm_flags))
				return NULL;
		}
	}
		...
		unsigned long addr, int *rss)
{
	...
	if (unlikely(!pte_present(pte))) {
		if (!pte_file(pte)) {
			...
			if (is_write_migration_entry(entry) &&
					ax8swap_is_cow_mapping(vm_flags)) {
				/*
	 ...
	 */
	if (ax8swap_is_cow_mapping(vm_flags)) {
 ...
 */
struct page *ax8swap_follow_page(struct vm_area_struct *vma, unsigned long address,
			unsigned int flags)
{
	pgd_t *pgd;
	pud_t *pud;
...
static inline int ax8swap_use_zero_page(struct vm_area_struct *vma)
{
	/*
		...
		struct page **pages, struct vm_area_struct **vmas)
{
	...
	do {
		...
		if (pages)
			foll_flags |= FOLL_GET;
		if (!write && ax8swap_use_zero_page(vma))
...
pte_t *ax8swap_get_locked_pte(struct mm_struct *mm, unsigned long addr,
			spinlock_t **ptl)
{
 ...
 */
static int ax8swap_insert_page(struct vm_area_struct *vma, unsigned long addr,
			...
			struct page *page)
{
	...
	return ax8swap_insert_page(vma, addr, page, vma->vm_page_prot);
}


static int ax8swap_insert_pfn(struct vm_area_struct *vma, unsigned long addr,
			unsigned long pfn, pgprot_t prot)
{
	...
	ax8swap_update_mmu_cache(vma, addr, entry); /* XXX: why not for insert_page? */
			...
			unsigned long pfn)
{
	...
	BUG_ON((vma->vm_flags & VM_PFNMAP) && ax8swap_is_cow_mapping(vma->vm_flags));
	...
	ret = ax8swap_insert_pfn(vma, addr, pfn, pgprot);
			...
			unsigned long pfn)
{
	...
	return ax8swap_insert_pfn(vma, addr, pfn, vma->vm_page_prot);
}
 ...
 */
static int ax8swap_remap_pte_range(struct mm_struct *mm, pmd_t *pmd,
			unsigned long addr, unsigned long end,
			unsigned long pfn, pgprot_t prot)
{
	pte_t *pte;
...
static inline int ax8swap_remap_pmd_range(struct mm_struct *mm, pud_t *pud,
			unsigned long addr, unsigned long end,
			unsigned long pfn, pgprot_t prot)
{
	pmd_t *pmd;
	...
	do {
		next = pmd_addr_end(addr, end);
		if (ax8swap_remap_pte_range(mm, pmd, addr, next,
...
static inline int ax8swap_remap_pud_range(struct mm_struct *mm, pgd_t *pgd,
			unsigned long addr, unsigned long end,
			unsigned long pfn, pgprot_t prot)
{
	pud_t *pud;
	...
	do {
		next = pud_addr_end(addr, end);
		if (ax8swap_remap_pmd_range(mm, pud, addr, next,
				pfn + (addr >> PAGE_SHIFT), prot))
			return -ENOMEM;
	} while (pud++, addr = next, addr != end);
	return 0;
		    ...
		    unsigned long pfn, unsigned long size, pgprot_t prot)
{
	...
	if (addr == vma->vm_start && end == vma->vm_end)
		vma->vm_pgoff = pfn;
	else if (ax8swap_is_cow_mapping(vma->vm_flags))
		return -EINVAL;
...
static int ax8swap_apply_to_pte_range(struct mm_struct *mm, pmd_t *pmd,
				     unsigned long addr, unsigned long end,
...
static int ax8swap_apply_to_pmd_range(struct mm_struct *mm, pud_t *pud,
				     unsigned long addr, unsigned long end,
				     pte_fn_t fn, void *data)
{
	pmd_t *pmd;
	...
	do {
		next = pmd_addr_end(addr, end);
		err = ax8swap_apply_to_pte_range(mm, pmd, addr, next, fn, data);
		if (err)
			break;
	} while (pmd++, addr = next, addr != end);
	return err;
...
static int ax8swap_apply_to_pud_range(struct mm_struct *mm, pgd_t *pgd,
				     unsigned long addr, unsigned long end,
				     pte_fn_t fn, void *data)
{
	pud_t *pud;
	...
	do {
		next = pud_addr_end(addr, end);
		err = ax8swap_apply_to_pmd_range(mm, pud, addr, next, fn, data);
		if (err)
			break;
	} while (pud++, addr = next, addr != end);
	return err;
			...
			unsigned long size, pte_fn_t fn, void *data)
{
	...
	do {
		next = pgd_addr_end(addr, end);
		err = ax8swap_apply_to_pud_range(mm, pgd, addr, next, fn, data);
		if (err)
			break;
	} while (pgd++, addr = next, addr != end);
...
static void ax8swap_reset_vma_truncate_counts(struct address_space *mapping)
{
	struct vm_area_struct *vma;
...
static int ax8swap_unmap_mapping_range_vma(struct vm_area_struct *vma,
...
static inline void ax8swap_unmap_mapping_range_tree(struct prio_tree_root *root,
					    struct zap_details *details)
{
	struct vm_area_struct *vma;
	struct prio_tree_iter iter;
...
restart:
	ax8swap_vma_prio_tree_foreach(vma, &iter, root,
		...
		if (ax8swap_unmap_mapping_range_vma(vma,
...
static inline void ax8swap_unmap_mapping_range_list(struct list_head *head,
					    struct zap_details *details)
{
	struct vm_area_struct *vma;
...
restart:
	list_for_each_entry(vma, head, shared.vm_set.list) {
		...
		if (ax8swap_unmap_mapping_range_vma(vma, vma->vm_start,
		...
		loff_t const holebegin, loff_t const holelen, int even_cows)
{
	...
	if (unlikely(is_restart_addr(mapping->truncate_count))) {
		if (mapping->truncate_count == 0)
			ax8swap_reset_vma_truncate_counts(mapping);
		mapping->truncate_count++;
	}
	details.truncate_count = mapping->truncate_count;

	if (unlikely(!prio_tree_empty(&mapping->i_mmap)))
		ax8swap_unmap_mapping_range_tree(&mapping->i_mmap, &details);
	if (unlikely(!list_empty(&mapping->i_mmap_nonlinear)))
		ax8swap_unmap_mapping_range_list(&mapping->i_mmap_nonlinear, &details);
 ...
 * We return with mmap_sem still held, but pte unmapped and unlocked.
 */
static int ax8swap___do_fault(struct mm_struct *mm, struct vm_area_struct *vma,
 ...
 */
int ax8swap___pud_alloc(struct mm_struct *mm, pgd_t *pgd, unsigned long address)
{
 ...
 * We've already handled the fast-path in-line.
 */
int ax8swap___pmd_alloc(struct mm_struct *mm, pud_t *pud, unsigned long address)
{
...
int ax8swap_follow_phys(struct vm_area_struct *vma,
 ...
 */
int ax8swap_access_process_vm(struct task_struct *tsk, unsigned long addr, void *buf, int len, int write)
{
	struct mm_struct *mm;
	struct vm_area_struct *vma;
	void *old_buf = buf;
 ...
 */
void ax8swap_print_vma_addr(char *prefix, unsigned long ip)
{
