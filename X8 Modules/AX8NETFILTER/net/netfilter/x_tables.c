				    ...
				    const char *name)
{
	...
	list_for_each_entry(t, &init_ax8netfilter_net.xt.tables[af], list)
#else
...
#ifdef CONFIG_AX8_NETFILTER
	list_for_each_entry(t, &init_ax8netfilter_net.xt.tables[table->af], list) {
#else
...
#ifdef CONFIG_AX8_NETFILTER
	list_add(&table->list, &init_ax8netfilter_net.xt.tables[table->af]);
#else
...
static void *xt_table_seq_start(struct seq_file *seq, loff_t *pos)
{
	...
	mutex_lock(&xt[af].mutex);
#ifdef CONFIG_AX8_NETFILTER
	return seq_list_start(&init_ax8netfilter_net.xt.tables[af], *pos);
#else
...
#ifdef CONFIG_AX8_NETFILTER
	return seq_list_next(v, &init_ax8netfilter_net.xt.tables[af], pos);
#else
...
#ifdef CONFIG_AX8_NETFILTER
		INIT_LIST_HEAD(&init_ax8netfilter_net.xt.tables[i]);
#else
