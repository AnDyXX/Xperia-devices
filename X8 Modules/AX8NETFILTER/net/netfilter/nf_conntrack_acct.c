...
static struct ctl_table acct_sysctl_table[] = {
	{
		...
		.data		= &init_ax8netfilter_net.ct.sysctl_acct,
#else
...
#ifdef CONFIG_AX8_NETFILTER
	table[0].data = &init_ax8netfilter_net.ct.sysctl_acct;

	init_ax8netfilter_net.ct.acct_sysctl_header = register_net_sysctl_table(net,
			nf_net_netfilter_sysctl_path, table);
	if (!init_ax8netfilter_net.ct.acct_sysctl_header) {
...
static void nf_conntrack_acct_fini_sysctl(struct net *net)
{
	struct ctl_table *table;
#ifdef CONFIG_AX8_NETFILTER
	table = init_ax8netfilter_net.ct.acct_sysctl_header->ctl_table_arg;
	unregister_net_sysctl_table(init_ax8netfilter_net.ct.acct_sysctl_header);
#else
...
#ifdef CONFIG_AX8_NETFILTER
	init_ax8netfilter_net.ct.sysctl_acct = nf_ct_acct;
#else
