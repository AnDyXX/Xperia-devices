...
#include "dlt001_cam_devdrv_table.c"

#define AX_MODULE_VER			"v001"
#define AX_MODULE_NAME			"ax10minicamera"
...
static int dlt001_camera_probe(const struct msm_camera_sensor_info *info, struct msm_sensor_ctrl *s)
{
	...
	msm_camio_clk_rate_set_ax(DLT001_DEFAULT_CLOCK_RATE);
...
static int __init dlt001_init(void)
{
		...
		)
	{
		...
		other = driver_find("msm_camera_dlt001", &platform_bus_type);
		...
		other = driver_find("dlt001_camera", &i2c_bus_type);
