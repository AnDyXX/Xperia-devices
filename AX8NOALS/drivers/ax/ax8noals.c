...
#define AX_MODULE_NAME			"ax8noals"
...
static void lm3530_mode_als(struct i2c_client* client, int mode)
{
	struct lm3530_data *data = i2c_get_clientdata(client);
	...
	switch(mode)
	{
		case ALS_AUTO:
			data->als_mode = mode;
			printk(KERN_DEBUG "%s: Auto ALS mode enabled\n", __func__);
			break;
		case ALS_OFF:
			printk(KERN_DEBUG "%s: ALS disabled\n", __func__);
			data->als_mode = mode;
			break;
	}
...
static int lm3530_update_brightness(struct i2c_client* client)
{
	...
	if (data->als_mode == ALS_OFF || data->mode == LM3530_MODE_I2C_PWM_ALS ||
...
static const struct cfg_value_map cfg_als_mode_table[] = {
	{"als_off", ALS_OFF},
	{"auto",  ALS_AUTO},
	{NULL, 0}
};
				   ...
				   const char *buf, size_t size)
{
	...
	if (mode == -EINVAL)
	{
		mode = lookup_table(cfg_als_mode_table, buf);
		if(mode == -EINVAL)
			return -EINVAL;
		lm3530_mode_als(data->client, mode);
		mode = data->mode;
	}
...
static int __init lm3530_init(void)
{
	...
	other = driver_find(DRV_NAME, &i2c_bus_type);
	...
	err = i2c_add_driver(&lm3530_driver);

	printk(KERN_ERR AX_MODULE_NAME ": switching ALS modes is enabled now :)\n");
